...
import 'package:election_admin_panel/core/reponsive/SizeConfig.dart';
import 'package:election_admin_panel/core/universal_widgets/no_internet_widget.dart';
...
abstract class Utility {
  ...
  static void printILog(dynamic message) {
    Logger().i('Election_admin_panel: $message');
  }
  ...
  static void printELog(String message) {
    Logger().e('Election_admin_panel: $message');
  }
  ...
  // static Future<bool> isNetworkAvailable() async => await InternetConnectionChecker().hasConnection;
  ...
  static void printResponseDetails(Response? response) {
    if (response != null) {
      ...
      if (isOkay) {
        printILog('Path: $path, Method: $method, Status Text: $statusText, Status Code: $statusCode, Query $query');
      } else {
        printELog('Path: $path, Method: $method, Status Text: $statusText, Status Code: $statusCode, Query $query');
      }
    }
  }

  /// get formated [DateTime] eg. 12 Jul 2021
  static String getDayAbbrMonthYear(DateTime dateTime) => '${DateFormat.d().format(dateTime)} ${DateFormat.yMMM().format(dateTime)}';
  ...
  static String getDateAndMonth(DateTime dateTime) => DateFormat('dd MMM').format(dateTime);

  static String getWeekDay(DateTime dateTime) => DateFormat.EEEE().format(dateTime);
  ...
  /// URL Launcher
  // static void launchURL(String url) async => await canLaunchUrl(Uri.parse(url)) ? launchURL(url) : showDialog(message: 'Could not open $url');

  /// Show error dialog from response model
  static void showInfoDialog({required String message, bool isSuccess = false}) async {
    await Get.dialog(
  ...
  static void showMessage(String? message, MessageType messageType, Function()? onTap, String actionName) {
    ...
    if (profilePhoto != null) {
      for (var file in profilePhoto) {
        formData.files.addAll([
          MapEntry('profilePhoto', await DIO.MultipartFile.fromFile(file.path, filename: file.path.split('/').last)),
        ]);
      }
    }
